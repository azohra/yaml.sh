...
function expression_parse_primary(    expression, name, step, argument, value, value_type, child, key, source, initial, update, variable, slice, start_expression, end_expression) {
    ...
    while (i <= length(value)) {
        ...
        if (substr(value, i + 1, 1) == "{") {
            ...
            if (name == "") {
                ...
                result = result ((!is_set || ENVIRON[name] == "") ? replacement : ENVIRON[name])
            } else if (substr(suffix, 1, 1) == "-") {
                replacement = substr(suffix, 2)
                result = result (is_set ? ENVIRON[name] : replacement)
            } else if (suffix != "") {
                result = result substr(value, i, closing + 2)
            } else {
...
function expression_evaluate(expression, input,    output, middle, left_stream, right_stream, single, kind, node, resolved, child, i, j, collection, key, predicate, matched, argument_stream, argument, result_node, variable, previous, had, accumulator, update_stream, start_index, end_index, size, interpolation, partial_count, next_count, partial, stage) {
    ...
    if (kind == "filename" || kind == "fileindex" || kind == "documentindex") {
        for (i = 1; i <= expression_stream_count[input]; i++) {
            ...
            if (kind == "filename") {
                expression_stream_push(output, expression_scalar(node_filename[node], "string"))
            } else if (kind == "fileindex") {
                expression_stream_push(output, expression_scalar(node_file_index[node] "", "int"))
            } else {
                expression_stream_push(output, expression_scalar(node_document[node] "", "int"))
            }
        }
        return output
    }
