...
function parse_core(value, source_line, tag, anchor,    node, inner, count, i, separator, raw_key, key, child, alias_name) {
    ...
    if (substr(value, 1, 1) == "[" && substr(value, length(value), 1) == "]") {
        ...
        for (i = 1; i <= count; i++) {
            if (i == count && flow_piece[i] == "") {
                delete flow_piece[i]
                continue
            }
            separator = find_top_level_colon(flow_piece[i], 1)
            if (!separator && (substr(flow_piece[i], 1, 1) == "\"" || substr(flow_piece[i], 1, 1) == sprintf("%c", 39))) {
            ...
            if (separator) {
                ...
                add_mapping(child, key, parse_value(substr(flow_piece[i], separator + 1), source_line, -1, 0), source_line, raw_key == "<<")
            } else {
...
function parse_mapping_into(value, parent, indent, source_line,    separator, raw_key, key, child, raw_value) {
...
function process_line(raw, source_line,    indent, text, clean, key_text, separator, root, marker_content, parent) {
    sub(/\r$/, "", raw)
...
function expression_parse_primary(    expression, name, step, argument, value, value_type, child, key, source, initial, update, variable) {
    if (expression_token_type == "dot") {
        ...
        if (name == "reduce") {
            ...
            name == "sort_by" || name == "group_by" || name == "unique_by" || name == "any_c" || name == "all_c") {
            expression_expect("left_parenthesis")
            ...
            name == "min" || name == "max" || name == "any" || name == "all") {
