...
One readable shell file for querying, transforming, and carefully editing YAML.
...
Examples:
  ...
  ysh -i '.image.tag = "stable"' service.yml
...
Documents:
  ...
  -i, --inplace           update YAML files, rolling back a failed write
      --check             report whether an edit would change files (0 clean, 1 changed, 2 error)
      --diff              preview edits as unified diffs without writing
      --preserve-only     refuse changes that cannot keep comments and formatting

Evaluation:
  -e, --exit-status       fail on no result, null, or false
      --explain           summarize selections, changes, and formatting behavior
      --explain=json      emit one JSON Lines summary per input without YAML values
...
The query language supports paths, streams, variables, slices, interpolation,
portable regexes, maps, reducers, sorting, arithmetic, construction, assignment,
deletion, deep merge, codecs, dynamic eval, and guarded file loads. Many paths
and operators will be familiar to yq users.
Collections emit JSON by default.
YAML edits keep comments and formatting when possible; --preserve-only refuses
the change when they cannot be kept.
EOF
}
