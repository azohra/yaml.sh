...
3. Run `make all` to rebuild, lint, and test the project. Parser and evaluator changes should also run `make fuzz`, `make presentation`, `make adversarial`, `make scale`, and the pinned conformance/differential gates. Update `test/public-contract.tsv` when the supported surface or its product role changes.
